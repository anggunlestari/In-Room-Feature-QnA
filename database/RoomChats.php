...
$sql = mysqli_query($conn, "SELECT * from chats WHERE id_session='$id_session'");
$cek = mysqli_num_rows($sql);

$url_sesi = "http://192.168.18.67:8001/items/session?fields=session_id,start_time,finish_time&filter[session_id]=" . $id_session;
$curl = curl_init();
...
if ($cek > 0) {
	if (new DateTime("2021-12-01T10:00:00") >= new DateTime($jam_mulai) && new DateTime("2021-12-01T11:00:00") < new DateTime($jam_selesai)) { // SUDAH BERJALAN
		...
		$sql2 = mysqli_query($conn, "UPDATE chats SET status='1' WHERE id_session=$id_session");
		echo "<script>alert('Sesi sudah selesai!');document.location.href='../error-page/error_jam_sudah.html';</script>";
	}
} else { // BELUM DIMULAI
	$sql1 = mysqli_query($conn, "INSERT into chats (id_session, status) VALUES ($id_session, 0)");
